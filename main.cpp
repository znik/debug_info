...
struct var_info_t {
	var_info_t *const instance() {
		static var_info_t * instance;
		return instance;
	}
	
	void add_var(const std::string& filepath, const int linenumber, const std::string& type,
		const std::string& name, const std::string& undecorated_name) {

		srcfile_t *srcfile = 0;
		for (unsigned i = 0; i < _srcfiles.size(); ++i) {
			if (filepath == _srcfiles[i]._path)
				srcfile = &_srcfiles[i];
		}
		if (0 == srcfile) {
			_srcfiles.push_back(srcfile_t());
			_srcfiles[_srcfiles.size() - 1]._path = filepath;
		}

		srcfile_t& src_file = *srcfile;
		
		var_t *const new_var =
			new var_t(linenumber, type, name, undecorated_name);
		src_file._storage.push_back(new_var);
		src_file._undecorated_name2var[undecorated_name] = new_var;
		src_file._name2var[name].push_back(new_var);
		src_file._linenumber2var[linenumber].push_back(new_var);
	}

	std::string get_var_type(const std::string& undecorated_name) {
		for (unsigned i = 0; i < _srcfiles.size(); ++i) {
			std::string &s =
				_srcfiles[i]._undecorated_name2var[undecorated_name]->_type;
			if (!s.empty())
				return s;
		}
		return std::string();
	...
	std::string get_var_type(const std::string& file_path,
		const int linenumber, const std::string& name) {

		for (unsigned i = 0; i < _srcfiles.size(); ++i) {
			if (file_path == _srcfiles[i]._path) {
				srcfile_t &srcfile = _srcfiles[i];
				std::vector<var_t*>& vlist =
					srcfile._linenumber2var[linenumber];
				for (unsigned j = 0; j < vlist.size(); ++j) {
					if (name == vlist[j]->_name)
						return vlist[j]->_type;
				}
				return std::string();		
			}
		}
...
protected:
	var_info_t() {}
	~var_info_t() {
		for (unsigned k = 0; k < _srcfiles.size(); ++k) {
			srcfile_t& srcfile = _srcfiles[k];
			for (unsigned i = 0; i < srcfile._storage.size(); ++i) {
				delete srcfile._storage[i];
			}
		}
	}
...
public:
	struct var_t {
		var_t(const int linenumber, const std::string& type,
			const std::string& name, const std::string& undecorated_name) :
			_linenumber(linenumber), _type_offset(-1), _type(type), _name(name),
			_undecorated_name(undecorated_name) {}

		int _linenumber;
		int _type_offset;
		std::string _type;
		std::string _name;
		std::string _undecorated_name;
	};

private:
	struct srcfile_t {
		std::string _path;
		std::vector<var_t*> _storage;
		std::map<std::string, var_t*> _undecorated_name2var;
		std::map<std::string, std::vector<var_t*> > _name2var;
		std::map<int, std::vector<var_t*> > _linenumber2var;
	};

private:
	std::vector<srcfile_t> _srcfiles;
};


#define VARINFO		var_info_t::instance()
